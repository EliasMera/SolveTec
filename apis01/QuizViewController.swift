...
class QuizViewController: UIViewController {
	...
	private let backgroundColor = UIColor(red: 44/255, green: 49/255, blue: 79/255, alpha: 1.0)
	private let foregroundColor = UIColor(red: 68/255, green: 76/255, blue: 124/255, alpha: 1.0)
	...
	override func viewDidLoad() {
		...
		let filePathCategorias = categoriasFilePath()
		...
		if !FileManager.default.fileExists(atPath: filePathCategorias) {
			print("No existe el categorias.plist, vamos a crearlo")
			
			let bundle = Bundle.main.path(forResource: "categorias", ofType: "plist")
			
			do {
				try FileManager.default.copyItem(atPath: bundle!, toPath: filePathCategorias)
			} catch {
				print(error)
			}
			
			print("Archivo categorias.plist creado")
		}
		
		if FileManager.default.fileExists(atPath: filePathUser) && FileManager.default.fileExists(atPath: filePathCategorias) {
			
			let dic = NSDictionary(contentsOfFile: filePathUser) as? [String: Any]
			let segundos = dic?["timer"] as! String?
			totalTime = Double(segundos!)!
			print("segundos: \(totalTime)")
			
			
			// Pone todo en 0
			let arrCategory = NSMutableArray(contentsOfFile: filePathCategorias)
			...
			arrCategory?.write(toFile: filePathCategorias, atomically: true)
			print("cargo todo en 0s")
...
//				print("cambio el plist - mal")
	...
	func showAlert(forReason reason: Int) {
		...
		case 2:
			quizAlertView = QuizAlertView(withTitle: "Fin del juego", andMessage: "Felicidades. Contestaste todo de manera correcta.", colors: [backgroundColor, foregroundColor])
		default:
			break
		}
	...
	func categoriasFilePath() -> String {
		let paths = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true)
		let documentsDirectory = paths[0]
		return documentsDirectory.appending("/categorias.plist")
	}
